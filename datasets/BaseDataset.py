...
class BaseDataset(Dataset, metaclass=abc.ABCMeta):
    ...
    def __len__(self):
        return int(self.cfg.series_per_data[0] * self.cfg.train_test_range[0]) + \
            int(self.cfg.series_per_data[1] * self.cfg.train_test_range[1]) + \
            int(self.cfg.series_per_data[2] * self.cfg.train_test_range[2])
